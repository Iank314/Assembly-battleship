...
piece_done:
    ...
    beq  $s2, 0, success       # No errors: return 0
    li   $t0, 1
    andi $t1, $s2, $t0         # Check if error code 1 (occupied) is present
    beq  $t1, 0, check_out_of_bounds
    li   $v0, 1                # Return 1 if only occupied error occurred
    j    piece_cleanup

check_out_of_bounds:
    li   $t0, 2
    andi $t1, $s2, $t0         # Check if error code 2 (out of bounds) is present
    beq  $t1, 0, mixed_error
